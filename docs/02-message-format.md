...
| 4 | 32 | `sender` | Solana pubkey (wallet or program PDA) that authorized this |
| 36 | 8 | `nonce` | per-`sender` counter from `SenderState` |
| 44 | 20 | `target` | Base contract address to call |
| 64 | 16 | `value` | wei of native ETH to attach (`uint128`) |
| 80 | 8 | `gasLimit` | gas for the inner call |
| 88 | 8 | `expiry` | unix seconds; `0` = never expires |
| 96 | 1 | `mode` | `0 = DIRECT`, `1 = ACCOUNT` |
| 97 | 4 | `calldataLen` | length in bytes (`uint32`) |
| 101 | n | `calldata` | ABI-encoded call, selector first |

Fixed header is 101 bytes. Minimum valid envelope with a bare selector is 105.
...
```rust
let mut buf = Vec::with_capacity(101 + calldata.len());
buf.push(VERSION);                       // 1
